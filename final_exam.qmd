...
2.  Statistical significance can also not be explained solely through Rsquared values.
3.  X2 and X1 together has a larger Rsquared compared to simply X1. This means that the varience of Y that is explained by X1 and X2 together is more than that what is explained solely by X1,
...
Yes, that might be an explanation of why the variable 'South' is negative.
...
Based on this, we can conclude that there is a slight but positive increase of literacy based on education spend.
...
Based on table results we can see that the P_value for the spending of education drops significantly such that it is not statically significant anymore. The estimate is still positive though. This indicates that the variable democratization is a confounding variable whose effects need to be taken into account if there is analysis to be done on the relationship between literacy and education spending.
...
In summary, while the intercept in your model is statistically significant, its practical interpretation should be approached with caution due to the improbability of the independent variables simultaneously taking on zero values in the real world. The value of the intercept is more informative when considered in conjunction with the coefficients of the independent variables.
...
```{r}
#Note explaining why the 0.95 and not 0.90 
qt(0.95, 45)
```
...
```{r}
lower_bound <- coef_south - t_value * SE_south
upper_bound <- coef_south + t_value * SE_south
```
...
The F-test in a multiple regression analysis is used to test the null hypothesis that all of the regression coefficients in the model are equal to zero. This hypothesis essentially posits that the model, as a whole, has no explanatory power in predicting the dependent variable.

In the context of your regression model, which includes the independent variables `cig_tax`, `college`, `union07`, and `south` to predict `obama08` (the percentage of a state's vote that President Obama received in the 2008 U.S. presidential election), the null hypothesis for the F-test can be stated as follows:
...
In other words, the F-test is checking whether there is any evidence to suggest that at least one of these independent variables is useful in predicting the dependent variable `obama08`. If the F-test's p-value is small (typically less than 0.05), it suggests that you can reject the null hypothesis and conclude that at least one of the coefficients is significantly different from zero, indicating that the model provides some explanatory power. If the p-value is not small, you would not reject the null hypothesis, suggesting that your model might not be useful for explaining the variation in the dependent variable.
